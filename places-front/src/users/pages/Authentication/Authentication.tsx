...
class Authentication extends React.Component<AuthenticationProps, AuthenticationState> {
  ...
  switchModeHandler = () => {
    ...
    if (!this.state.isLoginMode) {
      this.setState((oldState) => ({
        isLoginMode: !oldState.isLoginMode,
    ...
    this.setState((oldState) => ({
      isLoginMode: !oldState.isLoginMode,
      inputs: { email, password, name: { isValid: false, value: '' }, image: { isValid: false, value: null } },
      isValid: false,
    }));
  };
  ...
  performSignup = async () => {
    this.setState({ isLoading: true });
    try {
      const { email, password, name, image } = this.state.inputs;
